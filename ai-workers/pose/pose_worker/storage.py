...
class DownloadError(RuntimeError):
    """No se pudo descargar el archivo de video."""
...
class VideoDownloader:
    ...
    def download(self, video_url: str) -> Path:
        ...
        raise DownloadError(f"URL de video no soportada: {video_url}")
    ...
    def parse_s3_url(video_url: str) -> tuple[str, str]:
        ...
        if not bucket or not key:
            raise DownloadError(f"URL S3 invalida: {video_url}")
        return bucket, key
    ...
    def _download_s3(self, video_url: str) -> Path:
        try:
            ...
            raise DownloadError("boto3 no esta instalado para descargar archivos S3/MinIO") from exc
        ...
        try:
            ...
            return destination
        except Exception as exc:  # pragma: no cover - depende de credenciales/servicio externo.
            raise DownloadError(f"No se pudo descargar {video_url} desde S3/MinIO") from exc

    def _download_http(self, video_url: str) -> Path:
        try:
            import requests
        except Exception as exc:  # pragma: no cover - depende del entorno.
            raise DownloadError("requests no esta instalado para descargas HTTP") from exc

        parsed = urlparse(video_url)
        destination = self._destination_for_key(parsed.path)
        ...
        try:
            ...
            raise DownloadError(f"No se pudo descargar {video_url} por HTTP") from exc

    def _copy_local(self, source: Path) -> Path:
        if not source.exists() or not source.is_file():
            raise DownloadError(f"Archivo local no encontrado: {source}")
        destination = self._destination_for_key(source.name)
        shutil.copy2(source, destination)
        return destination

    def _destination_for_key(self, key: str) -> Path:
        suffix = Path(key).suffix or ".webm"
        filename = f"pose-input-{uuid.uuid4().hex}{suffix}"
